...
export const Filters: React.FC<Props> = ({ className }) => {
    ...
    <div>
      <div className="flex justify-between mt-4">
        ...
        <Input onChange={(e) => onChangeDate(e.target.value)} type="date" className='cursor-pointer' />
        {
            (artist || location || date) &&
            <Button onClick={() => { setArtist(''); setLocation(''); setDate(''); filterShows('', '', ''); }} className='ml-4 cursor-pointer'>Clear</Button>
        }
      </div>
    </div>
    );
